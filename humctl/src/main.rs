...
fn status() -> Result<()> {
    ...
    let s = Command::new("systemctl")
        .args(["--user", "status", "--no-pager", "humd"])
        .status()?;
    #[cfg(target_os = "macos")]
    let s = {
        let uid = unsafe { geteuid() };
        Command::new("launchctl")
            .args(["print", &format!("gui/{uid}/humd")])
            .status()?
    };
...
fn health() -> Result<()> {
    ...
    if !sock.exists() {
        bail!("socket file missing: {}", sock.display());
    }
    let mut s =
    ...
    match s.read(&mut buf) {
        ...
        Ok(_) => {
            println!("humd: ✓ live at {}", sock.display());
            Ok(())
        }
