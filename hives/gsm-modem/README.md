---
title: "gsm-modem"
description: "hum-on-a-burner-phone — USB GSM modem bee driven by AT commands"
---

# gsm-modem

> _hum-on-a-burner-phone — USB GSM modem bee driven by AT commands_

A bee that turns a cheap USB GSM dongle into a hum agent. SMS in
...
You can pipe AT commands at the bee via a fake serial pair using
`socat`:
...
- **No PDU mode.** Text-mode (`AT+CMGF=1`) only — fine for plain ASCII
  ...
  PDU-encoded payload, which this bee does not implement.
...
- **No SIM PIN unlock.** If your SIM requires a PIN, send `AT+CPIN=<pin>`
  manually before starting the bee (or extend `init()`).
...
similar to this bee but the transport is HTTP, not serial.
