...
//! nestling; C calls `nestling_discover("market-maker")` and must
//! receive `(A's HumdId, manifest)` within a short timeout. Then A
//! retracts; via the raw `nestling_announcements()` stream, C must
//! see a `Retract` envelope.

use std::time::Duration;

use ensemble::{
    nestlings::{NestlingAnnounce, NestlingManifest, Propensity},
...
async fn advertise_percolates_and_discover_filters_by_name() {
    ...
    let mut discover = ens_c.nestling_discover("market-maker");
    let mut raw = ens_c.nestling_announcements();

    // Wait for handshakes + topic subscription to wire up before A advertises.
    tokio::time::sleep(Duration::from_millis(30)).await;

    // Noise: an unrelated nestling — must not appear in the filtered receiver.
    ens_a
        .nestling_advertise(NestlingManifest::new("openai-server", "0.2.0", "0.7.0"))
        .await;

    // Real advertise.
    let manifest = NestlingManifest::new("market-maker", "0.1.0", "0.7.0")
    ...
    let (got_id, got_manifest) = timeout(Duration::from_millis(300), discover.recv())
        .await
        .expect("C did not receive nestling-advertise within 300ms")
    ...
    let env = timeout(Duration::from_millis(300), async {
        ...
        loop {
            ...
            if matches!(v, NestlingAnnounce::Retract { .. }) {
                return v;
            }
        }
    })
    ...
    match env {
        NestlingAnnounce::Retract { humd_id, name } => {
