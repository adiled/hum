//! `bp7-nestling` — DTN bridge to hum. RFC 9171 Bundle Protocol v7.
//!
...
//!
//! This nestling is a **leaf node** — we terminate bundles addressed
...
//! timeout on the nestling side. The DTN router on the other end
...
const NESTLING_NAME: &str = "bp7-nestling";
...
async fn main() -> Result<()> {
    ...
    info!(listen = %cfg.listen, eid = %cfg.node_eid, "bp7-nestling.listen");
...
async fn handle_bundle(
    ...
    let bndl = bp7::Bundle::try_from(bytes.as_slice())
...
fn build_reply(my_eid: &str, source_eid: &str, payload: &[u8]) -> bp7::Bundle {
    let src = bp7::EndpointID::try_from(my_eid).expect("our EID parses");
    let dst = bp7::EndpointID::try_from(source_eid).unwrap_or_else(|_| {
        warn!(source_eid, "could not parse source EID; reply will go to dtn:none");
        bp7::EndpointID::none()
    });
    bp7::bundle::new_std_payload_bundle(src, dst, payload.to_vec())
}
...
async fn run_prompt(
    ...
    let hello = json!({
        ...
        "source": "https://github.com/adiled/hum/tree/main/nestlings/bp7-nestling",
    });
