...
canonical Go nestling in this repo.

A nestling that turns a Twilio phone number into a hum agent. Send an
SMS to your Twilio number → Twilio POSTs to this nestling → the
...
nestling (its thrum socket is local-only by default — that's fine,
the nestling itself terminates the SMS, hum stays private).

## Configure (this nestling)
...
- **No outbound-initiated chat.** This nestling only answers; humd
